...
export const receiveGame = (game: Game): GameActionTypes => ({
  type: RECEIVE_GAME,
  game,
})
...
export const createGame = (
  ...
  api.createGame(teams).then(({ data }) => {
...
export const joinTeam = (
  ...
  api.joinTeam(gameId, teamId).then(() => {
...
export const createPhrase = (
  ...
  api.createPhrase(text, gameId).then(({ data }) => {
...
export const submitPlayedPhrases = (
  ...
  // dispatch: ThunkDispatch<{}, {}, AnyAction>,
  {
    api.submitPlayedPhrases({ gameId, playedPhrases }).then(() => {
      if (callback) callback()
    })
  }
